		...
		<section className="relative overflow-hidden bg-[#F8FBFF] py-10">
			<div className="absolute top-0 left-0 w-full h-full pointer-events-none opacity-30">
				<div
					...
					style={{
						background:
							"radial-gradient(circle, rgba(0, 138, 221, 0.1) 0%, rgba(248, 251, 255, 0) 70%)",
					}}
				/>
			</div>
				...
				<div className="grid gap-12 lg:grid-cols-12 lg:items-center">
					<div className="hidden lg:col-span-3 lg:flex flex-col gap-8 justify-center">
						<motion.div
							onClick={() => {
								setDirection(-1);
								setActiveIndex(getRelativeIndex(-1));
							}}
						...
						>
							...
							<div className="min-w-0">
								<h4 className="truncate font-outfit text-sm font-bold text-[#0D1117]">
									{TESTIMONIALS[getRelativeIndex(-1)].author}
								</h4>
								<p className="truncate text-[10px] uppercase tracking-wider text-[#6B7280]">
									{TESTIMONIALS[getRelativeIndex(-1)].company}
								</p>
							</div>
						</motion.div>
						...
						<div className="relative pl-4">
							...
							<div className="flex items-center gap-4 p-6 rounded-2xl border border-[#008ADD]/20 bg-white shadow-2xl">
								...
								<div className="min-w-0">
									<h4 className="truncate font-outfit text-base font-bold text-[#0D1117]">
										{activeTestimonial.author}
									</h4>
									<div className="flex gap-0.5 mt-1 text-[#F59E0B]">
										{[...Array(5)].map((_, i) => (
											<StarIcon key={i} size={12} fill="#F59E0B" />
										))}
									</div>
								</div>
							</div>
							...
							<AnimatePresence mode="wait" custom={direction}>
								...
								>
									...
									<div className="text-center lg:text-left">
										<h3 className="font-outfit text-2xl font-bold text-[#0D1117]">
											{activeTestimonial.author}
										</h3>
										<p className="font-outfit text-base font-medium text-[#008ADD]">
											{activeTestimonial.position}
										</p>
										<div className="mt-4 flex justify-center lg:justify-start gap-1 text-[#F59E0B]">
											{[...Array(5)].map((_, i) => (
												<StarIcon key={i} size={20} fill="#F59E0B" />
											))}
										</div>
									</div>
								</motion.div>
