				...
				<div className="mb-20 text-center">
					<FadeInUp>
						...
						<h2 className="mb-6 font-outfit text-[40px] font-bold leading-tight text-[#0D1117] sm:text-[48px] lg:text-[56px]">
							Growing Fast Should <br className="hidden sm:block" /> Not Feel
							Chaotic
						</h2>
						<p className="mx-auto max-w-2xl font-outfit text-lg font-medium leading-relaxed text-slate-500">
							If any of these sound familiar, you are not alone. Most IT
							companies hit these walls between 10 and 250 employees.
						</p>
					</FadeInUp>
				</div>
							...
							<div
								className={`group relative rounded-2xl p-8 transition-all duration-500 hover:-translate-y-2 ${
									problem.highlight
										...
										: "bg-white border border-slate-100 shadow-[0_20px_50px_-15px_rgba(0,0,0,0.05)]"
								} hover:bg-[#008ADD] hover:shadow-2xl hover:shadow-blue-500/30 hover:border-transparent`}
							>
								{/* Overlapping Icon Container */}
								<div
									className={`absolute -top-6 left-8 flex h-14 w-14 items-center justify-center rounded-full border-4 border-white transition-transform duration-500 group-hover:scale-110 group-hover:rotate-12 shadow-lg ${
										problem.highlight
											? "bg-white text-[#008ADD]"
											: "bg-[#FEE2E2] text-[#EF4444]"
									} group-hover:bg-white group-hover:text-[#008ADD]`}
								>
								...
								<div className="mt-4">
									<h3
										className={`mb-3 font-outfit text-2xl font-bold leading-tight ${
											problem.highlight ? "text-white" : "text-[#1E293B]"
										} group-hover:text-white`}
									>
										{problem.title}
									</h3>
									<p
										className={`font-outfit text-base font-medium leading-relaxed ${
											problem.highlight ? "text-blue-50" : "text-slate-500"
								...
								<div
									className={`absolute inset-0 pointer-events-none overflow-hidden rounded-2xl transition-opacity ${problem.highlight ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
								>
