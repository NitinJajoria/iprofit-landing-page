...
const outcomes = [
	{
		...
		description:
			"Automate repetitive tasks across HR, Finance, and Operations.",
	...
	},
	{
		...
		description:
			"One entry, instant reflection everywhere - no duplicate work.",
	...
	},
	{
		...
		description:
			"Standardized workflows reduce errors and improve output quality.",
	...
	},
	{
		...
		description:
			"Built-in Indian compliance for GST, TDS, PF, and payroll filings.",
		...
		<section className="bg-white py-10 lg:py-16">
			<div className="container mx-auto px-4 lg:px-8">
				...
				<div className="mb-20 text-center">
					...
					>
						Outcomes That Actually <br className="hidden sm:block" /> Move the
						Needle
					</motion.h2>
				</div>

				{/* Bento Grid */}
				<div className="mx-auto max-w-7xl grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-12 auto-rows-fr">
					{outcomes.map((item, idx) => (
						<motion.div
							...
							className={`relative overflow-hidden rounded-[2.5rem] p-6 sm:p-8 ${item.color} ${item.size} group transition-all duration-500 hover:shadow-2xl hover:shadow-gray-200/50 hover:-translate-y-1`}
						>
							{(() => {
								const isWideCard = item.size.includes("lg:row-span-1");
								return (
									<div className="relative z-10 flex h-full flex-col justify-between">
										<div
											className={isWideCard ? "flex items-center gap-4" : ""}
										>
											<div
												className={`mb-4 flex h-12 w-12 items-center justify-center rounded-2xl ${item.iconColor} ${item.textColor} transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 shadow-sm ${isWideCard ? "mb-0 flex-shrink-0" : ""}`}
											>
												{item.icon}
											</div>
											<h3
												className={`font-outfit text-xl font-bold leading-tight ${item.textColor} sm:text-2xl ${isWideCard ? "mb-0" : ""}`}
											>
												{item.title}
											</h3>
										</div>
										<p
											className={`font-outfit text-base font-medium leading-relaxed ${item.textColor} opacity-60`}
										>
											{item.description}
										</p>
									</div>
