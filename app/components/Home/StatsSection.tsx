						...
						{stats.map((stat, idx) => (
							<FadeInUp key={stat.label} delay={idx * 0.1}>
								<div className="group flex flex-col items-center rounded-[2rem] bg-white p-8 text-center transition-all duration-500 hover:shadow-2xl hover:shadow-blue-500/10">
									{/* Icon */}
									<div
										className={`mb-8 flex h-14 w-14 items-center justify-center rounded-2xl ${stat.bgColor} ${stat.color} transition-all duration-500 group-hover:scale-110 group-hover:rotate-6`}
									>
									...
									{/* Number */}
									<div
										className={`mb-3 font-outfit text-3xl font-black lg:text-4xl ${stat.color}`}
									>
										{stat.number}
									</div>
